...
#		  ['GW MC',0xa0,0x00,0x00,0x00,0x04,0x10,0x10,0xaa,0x54,0x30,0x32,0x00,0xff,0x01,0xff,0xff]
		]
...
# new apdu primitives for Google Wallet
GET_VERSION = [0x9F,0x7E] # returns 9F 7E 30 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 30 31 38 31 30 39 30 33 31 34 4A 43 4F 50 76 32 34 31 52 33 90 0
PRIM_1 = [0x9F,0x17] # returns 9F 17 01 00 90 0
PRIM_2 = [0x9F,0x23] # returns 9F 17 01 00 90 0
# this one is just log format
#PRIM_3 = [0x9F,0x4F] # returns 9F 4F 11 9F 27 01 9F 02 06 5F 2A 02 9A 03 9F 36 02 9F 52 06 90 0
...
TAGS= 	{	
	...
	0x73:['GW Record Template?',BINARY,TEMPLATE],
	...
	0x56:['Google Track 2 Data',TEXT,ITEM],
	...
	0x9f62:['GW: 2',BINARY,ITEM],
	0x9f63:['GW: 3',BINARY,ITEM],
	0x9f64:['GW: 6',BINARY,ITEM],
	0x9f65:['GW: 4',BINARY,ITEM],
	0x9f66:['Card Production Life Cycle',BINARY,ITEM],
	0x9f67:['GW: 8',BINARY,ITEM],
	0x9f6b:['Google PAN',BINARY,ITEM],
	0x9f6c:['GW: 1',BINARY,ITEM],
	0x9f6e:['GW: 5',BINARY,ITEM],
	0x9f7e:['GW Version:',TEXT,ITEM],
...
#		print "%02x" % b,
	...
	if sw1 == SW1_WRONG_LENGTH:
		print "| command used wrong length. retry with correct length."
		apdu= apdu[:len(apdu) - 1] + [sw2]
		return send_apdu(apdu)
	if sw1 == SW1_RESPONSE_BYTES:
		print "| response bytes available."
	...
	# for b in response:
	# 	print "%02x" % b,
	# print "sw1 is %02x; sw2 is %02x" % (sw1, sw2)
	# print "|--------DEBUG--------\n"
...
def send_single_command(command):
	...
	if len(command.split()) == 1:
		for i in range(0,len(command),2):
			tmp.append(int(command[i:i+2],16))
			i += 1
		apdu = tmp
	else:
		for b in command.split():
			apdu.append( int(b,16) ) # messy convert to 0x00 format
	response, sw1, sw2= send_apdu(apdu)
	return response
...
try:
	...
	opts, args  = getopt.getopt(sys.argv[1:],'aAdefoprstv')
	for o, a in opts:
		if o == '-a':
			BruteforceAID= True
		if o == '-A':
...
PIN= ''
if args:
	if not args[0].isdigit() and not SingleCommand:
...
try:
	...
	# try to select PSE
	print "Eliciting PSE from wallet"
		...
		while current < len(aidlist):
			...
			if selected:
				...
				if SingleCommand:
					command = args[0]
					while not( command == '\n' or command == EOF):
						print 'Sending single command',command
						response= send_single_command(command)
						if response:
							decode_pse(reponse)
						command = raw_input('Please enter next command\n')
					print 'End single command'
				else:
					...
					if ret:
						print '  Log Format: ',
						hexprint(logf)
					ret, length, vers= get_primitive(GET_VERSION)
					if ret:
						print '  Version: ',
						textprint(vers)
					ret, length, prim1= get_primitive(PRIM_1)
					if ret:
						print '  Primitive 1: ',
						hexprint(prim1)
					ret, length, prim2= get_primitive(PRIM_2)
					if ret:
						print '  Primitive 2: ',
						hexprint(prim2)
					current += 1
